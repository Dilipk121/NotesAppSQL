...
class NoteDataBaseHelper(context: Context) : SQLiteOpenHelper(
    ...
    fun insertNotes(note: Note) {
        val db = writableDatabase // it can be modify
    ...
    fun getAllNotes():List<Note>{
        ...
        while (cursor.moveToNext()){
            ...
            val _title = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_TITLE))
            val _content = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_CONTENT))
            val _date = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_DATE))

            val note = Note(_id,_title,_content,_date)
            notesList.add(note)
        }
        cursor.close()
        db.close()
